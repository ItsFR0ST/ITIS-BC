...
client.user.setGame(`$bc |ITIS BC`,"http://twitch.tv/ITIS_ME_BC)_Live")
...
client.on("message", async message => {
    ...
    command = command.slice(prefix.length);
        if(!message.channel.guild) return;
            ...
            if(command == "bc") {
                // if(!message.member.hasPermission("ＩＴＩＳ ＭＥ")) {
                    return message.channel.send("**للأسف لا تمتلك صلاحية `ＩＴＩＳ ＭＥ`**");
                }
                    if(!args) {
...
client.on("message", async message => {
    if(message.content == prefix + "server") {
        if(!message.channel.guild) return;
            if(!message.member.hasPermission("مدير السيرفر")) {
...
client.on("message", async message => {
    if(message.content.startsWith(prefix + "help")) {
        let help = new Discord.RichEmbed()
            ...
            Itis Me رابط سيرفر : https://discord.gg/awWwcWZ
            **`);
            message.channel.sendEmbed(help); // رابط السيرفر يعود الى سيرفر @αℓɪyєαн⁶⁶ ♔#5162  .
    }
});
